...
def compute_reward(actions):
    ...
    # STRONGER SIGNAL
    return 5.0 * similarity + 0.1 * diversity
...
for step in range(STEPS):
    ...
    # ======================
    # ADVANTAGE (CRITICAL FIX)
    # ======================
    advantage = reward_used - reward_used.detach()
    ...
    total_loss_tensor += ROLE_BETA * role_loss
    ...
    loss_log.append(total_loss_tensor.item())
    ...
    if step % 100 == 0:
        print(f"Step {step} | Reward {reward:.3f} | Coop {coop:.3f} | "
              ...
              f"Loss {total_loss_tensor.item():.3f}")
