...
def compute_reward(actions):
    ...
    return similarity + 0.1 * diversity
...
def role_diversity(policies):
    return torch.var(torch.stack(policies)).item()
...
for step in range(STEPS):
    ...
    # prevent collapse at high cooperation
    if coop > 0.9:
        reward_used += DIVERSITY_BONUS * act_div
    ...
    for i in range(NUM_AGENTS):
        ...
        loss = -reward_used * log_prob - ENTROPY_BETA * ent
    ...
    # occasional rollback if collapse
